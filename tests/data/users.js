...
const users = async () => {
  ...
  await Promise.all(data.map(async (user) => {
    ...
    const orgs = await Promise.all(user.organizations.map(async (o) => OrganizationModel.OrganizationModel.findOne({ name: o })));
    const teams = await Promise.all(user.teams.map(async (team) => TeamModel.TeamModel.findOne({ name: team })));
