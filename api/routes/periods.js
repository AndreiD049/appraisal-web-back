...
appraisalPeriodsRouter.get(
  '/',
  AuthorizeReq('APPRAISAL PERIODS', 'read'),
  async (req, res, next) => {
    try {
...
appraisalPeriodsRouter.post(
  '/',
  AuthorizeReq('APPRAISAL PERIODS', 'create'),
  async (req, res, next) => {
    try {
      const { body } = req;
...
appraisalPeriodsRouter.post(
  '/:id/finish',
  AuthorizeReq('APPRAISAL PERIODS', 'finish'),
  async (req, res, next) => {
    try {
      const periodId = req.params.id;
 ...
 */
appraisalPeriodsRouter.post(
  '/:periodId/users/:userId/toggle-lock',
  AuthorizeReq(ADO.code, ADO.grants.toggleLock),
  async (req, res, next) => {
    try {
...
appraisalPeriodsRouter.get(
  '/:periodId/items/:id',
  AuthorizeReq('APPRAISAL DETAILS', 'read'),
  async (req, res, next) => {
    try {
...
appraisalPeriodsRouter.post(
  '/:id/items',
  AuthorizeReq('APPRAISAL DETAILS', 'create'),
  async (req, res, next) => {
    try {
      const periodId = req.params.id;
      const { body } = req;
      ...
      if (!body.type) next(new Error('Type should be specified'));
        ...
        throw new Error(
          `Request body and query have different periodId: ${body.periodId} vs ${periodId}`,
        );
...
appraisalPeriodsRouter.put(
  '/:periodId/items/:id',
  AuthorizeReq('APPRAISAL DETAILS', 'update'),
  async (req, res, next) => {
    try {
      const itemId = req.params.id;
      const { periodId } = req.params;
...
appraisalPeriodsRouter.delete(
  '/:periodId/items/:id',
  AuthorizeReq('APPRAISAL DETAILS', 'delete'),
  async (req, res, next) => {
    try {
      const itemId = req.params.id;
...
appraisalPeriodsRouter.get(
  '/:periodId/users/:userId',
  AuthorizeReq('APPRAISAL DETAILS - OTHER USERS', 'read'),
  async (req, res, next) => {
    try {
      const { periodId } = req.params;
      const { userId } = req.params;
      ...
      const items = (await AppraisalService.getUserItemsByPeriodId(periodId, user.id)).map((item) =>
        item.toJSON(),
      );
      period.items = items;
      res.json(period);
...
appraisalPeriodsRouter.post(
  ...
  AuthorizeReq('APPRAISAL DETAILS - OTHER USERS', 'create'),
  async (req, res, next) => {
    try {
      const periodId = req.params.id;
      const { userId } = req.params;
...
appraisalPeriodsRouter.put(
  '/:periodId/users/:userId/items/:id',
  AuthorizeReq('APPRAISAL DETAILS - OTHER USERS', 'update'),
  async (req, res, next) => {
    try {
      const itemId = req.params.id;
      const { userId } = req.params;
      ...
      if (body.user !== userId)
        throw new Error(
          `Request body and query have different user ids: ${body.user} vs ${userId}`,
        );
...
appraisalPeriodsRouter.delete(
  '/:periodId/users/:userId/items/:id',
  AuthorizeReq('APPRAISAL DETAILS - OTHER USERS', 'delete'),
  async (req, res, next) => {
    try {
      const itemId = req.params.id;
      const { userId } = req.params;
      ...
      if (String(item.periodId) !== periodId)
        throw new Error(
          `Request body and query have different periodId: ${item.periodId} vs ${periodId}`,
        );
      else if (String(item.user) !== userId)
        throw new Error(
          `Request body and query have different user ids: ${item.user} vs ${userId}`,
        );
