...
taskFlowRouter.get('/', AuthorizeReq(TASK_FLOW.code, TASK_FLOW.grants.read), async (req, res, next) => {
  try {
    ...
    const result = await TaskService.getTaskFlows(filter, req.user);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
taskFlowRouter.post('/', AuthorizeReq(TASK_FLOW.code, TASK_FLOW.grants.create), async (req, res, next) => {
  try {
    const data = res.body;
    const result = await TaskService.createTaskFlow(data, req.user);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
taskFlowRouter.delete('/:id', AuthorizeReq(TASK_FLOW.code, TASK_FLOW.grants.delete), async (req, res, next) => {
  try {
    const { id } = req.params;
    const result = await TaskService.deleteTaskFlow(id, req.user);
    res.json(result);
  } catch (err) {
    next(err);
  }
